...
Replace `/path/to/jerednel/mnemosyne` with your checkout path throughout.

## Claude Code

```bash
claude mcp add mnemosyne -- uv run --directory /path/to/jerednel/mnemosyne mnemosyne-server
```
...
```json
{
  "mcpServers": {
    "mnemosyne": {
      "command": "uv",
      "args": ["run", "--directory", "/path/to/jerednel/mnemosyne", "mnemosyne-server"]
    }
  }
}
```
